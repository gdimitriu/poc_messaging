...
public class PopulateAllies {
	...
	/**
	 * create the connection to sqs as jms and sns
	 * @throws JMSException
	 */
	...
	public void populateQueue() throws JMSException {
		...
		try {
			...
			Queue queue = session.createQueue(Constants.ALLIES + ".fifo");
		...
		PublishRequest publishRequest = new PublishRequest().withMessage(Constants.COMMAND_ALLIES).withTopicArn(commandTopicArn).withSubject(Constants.COMMAND_ALLIES);
	...
	public static void main(String[] args) {
		PopulateAllies allies = new PopulateAllies();
		try {
			allies.createConnection();
			allies.populateQueue();
		} catch (JMSException ex) {
			ex.printStackTrace();
		}
	}
